...
values (
    'same_named_entity',
    (select id from public_api.document_metadata_definition where name = 'legislation_named_entities'),
    (select id from public_api.document_metadata_definition where name = 'legislation_named_entities'),
    $q$
        select id_a as doc_id_a,
            ...
            jsonb_build_object('same_named_entity',data) as relationship_properties
        from (
...
select update_document_graph_relationship_definition((select id from public_api.document_graph_relationship_definition where name = 'same_named_entity'));